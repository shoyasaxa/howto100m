...
- m2e2_train.py is the script used to finetune the pre-trained embedding AND evaluate it on the manually labeled event co-reference dataset
- visualize_output.ipynb shows outputs of the text-video retrieval on the test set
- Youtube ASR Dataset Retrieval and Preprocessing.ipynb has functions I used to get ASR from youtube and process/format them to be used in the training scripts
- m2e2_train_with_ASR.py finetunes the embedding on the youtube captions and evaluates performance on the event-co-reference dataset
